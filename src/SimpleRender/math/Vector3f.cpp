...
const float dot(const Vector3f &a ,const Vector3f &b) {
	float result = 0;
	for (int i = 0; i < 3; i++) {
...
const Vector3f scalar_by(const Vector3f &a, const float k) {
	return Vector3f(a.data[0] * k, a.data[1] * k, a.data[2] * k);
}

const Vector3f operator * (const Vector3f &a, const float k) {
	return scalar_by(a, k);
}

const Vector3f normalize(const Vector3f &a) {
	// cannot be zero vector
	float div = sqrt(a.data[0] * a.data[0] + a.data[1] * a.data[1] + a.data[2] * a.data[2]);
	div = 1. / div;
...
const float mol_length(const Vector3f &a) {
	float r = a.data[0] * a.data[0] + a.data[1] * a.data[1] + a.data[2] * a.data[2];
	return sqrtf(r);
}